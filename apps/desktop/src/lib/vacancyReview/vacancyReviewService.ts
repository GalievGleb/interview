...
export function analyzeVacancyMock(input: VacancyReviewInput): VacancyAnalysis {
  ...
  return {
    ...
    hasLegend: Boolean(input.legendText?.trim()),
    resumeSource: input.resumeText?.trim() ? input.resumeSource : undefined,
  ...
  if (text) {
    try {
      ...
      return {
        ...
        answerStrategy: r.answerStrategy || undefined,
        whyThisAnswerWorks: r.whyThisAnswerWorks?.length ? r.whyThisAnswerWorks : undefined,
        deliveryTips: r.deliveryTips?.length ? r.deliveryTips : undefined,
