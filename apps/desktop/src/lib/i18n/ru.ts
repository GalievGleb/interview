...
export const ru = {
  ...
  'home.check.resume.packReady': 'контекст опыта готов — live отвечает вашими фактами',
  'home.check.resume.docs': 'документы добавлены, контекст опыта готовится',
  ...
  'docs.add.title': 'Добавьте материал',
  ...
  'history.analysis.empty':
    'Для этой сессии ещё нет персонального разбора. ИИ оценит только то, что действительно было сказано, и добавит подтверждение в результаты реальных интервью.',
  ...
  'prep.match.unknown': 'Не оценено без резюме',
  'prep.match.unknownHelp': 'Требования выделены, но опыт пока не оценивался',
  ...
  'prep.analysis.heuristicTimeout': 'AI-разбор не успел завершиться. Локальный план сохранил явные требования вакансии; можно повторить запрос.',
  'prep.analysis.heuristicOffline': 'Сервис AI сейчас недоступен. Показан локальный план по явным требованиям вакансии.',
  'prep.analysis.heuristicQuota': 'AI-разбор недоступен из-за лимита. Показан локальный план по явным требованиям вакансии.',
  'prep.analysis.heuristicEmpty': 'AI не вернул темы. Показан локальный план по явным требованиям вакансии.',
  ...
  'prep.smoke.finalizingHint': 'Обычно это занимает несколько секунд.',
  'prep.smoke.voiceAnswer': 'Голосовой ответ',
  'prep.smoke.recordingHint': 'Говорите как на интервью. Запись можно прослушать.',
  'prep.smoke.voiceHint': 'Запишите ответ или введите его текстом.',
  'prep.smoke.recordingReady': 'Запись готова',
  'prep.smoke.previewLabel': 'Прослушать записанный ответ',
  'prep.smoke.finishRecording': 'Завершить запись',
  'prep.smoke.transcribeAndScore': 'Распознать и оценить',
  'prep.smoke.rerecord': 'Перезаписать',
  ...
  'prep.smoke.startRec': 'Записать ответ',
  ...
  'prep.smoke.alreadyGood': 'Уже хорошо',
  'prep.smoke.improveFirst': 'Сначала исправить',
  ...
  'overlay.recap.openFullAnalysis': 'Открыть полный разбор',
  ...
  'overlay.rec.pauseTip': 'Поставить запись на паузу',
  'overlay.rec.resumeTip': 'Продолжить запись',
  'overlay.rec.pauseAria': 'Поставить запись на паузу',
  'overlay.rec.resumeAria': 'Продолжить запись',
  ...
  'overlay.clickThroughTip': 'Полностью пропускает клики сквозь оверлей. Ctrl+Alt+0 (цифра ноль) включает режим и возвращает управление. Ctrl+Alt+O (латинская буква) также работает',
  ...
  'overlay.sourceHealth.systemSilent':
    'Системный звук не поступает. На macOS проверьте доступ к записи экрана и системного аудио.',
  ...
  'settings.sub.account': 'Вход, подписка и устройства вашего профиля.',
  'settings.sub.ai': 'Модели для live-подсказок и разбора вакансий.',
  'settings.sub.speech': 'Выберите микрофон и проверьте уровень сигнала.',
  ...
  'account.title': 'Профиль SkillCue',
  'account.subtitle': 'Подписка привязана к профилю и работает на двух ваших устройствах.',
  'account.google': 'Продолжить с Google',
  'account.emailFallback': 'Войти или зарегистрироваться по почте',
  'account.email': 'Электронная почта',
  'account.password': 'Пароль',
  'account.code': 'Код из письма',
  'account.create': 'Создать профиль',
  'account.login': 'Войти',
  'account.haveAccount': 'У меня уже есть профиль',
  'account.needAccount': 'Создать новый профиль',
  'account.verify': 'Подтвердить почту',
  'account.resend': 'Отправить код ещё раз',
  ...
  'account.noPlan': 'Нет активной подписки',
  'account.unavailable': 'Профили ещё не включены в этой сборке. Локальные данные и старая лицензия продолжают работать.',
  'account.loading': 'Загружаю профиль…',
  'account.error': 'Не удалось выполнить действие. Попробуйте ещё раз.',
  'account.deviceLimit': 'Лимит — два устройства. Отключите одно из старых устройств и повторите вход.',
  'account.codeSent': 'Код отправлен на почту.',
  'account.resetDone': 'Пароль изменён. Теперь можно войти.',
  'backup.title': 'Резервная копия',
  'backup.desc': 'Сохраните профиль, резюме, историю, настройки и черновики перед переустановкой. Пароли, cookies и ключи входа в копию не попадают.',
  'backup.export': 'Сохранить копию',
  'backup.import': 'Восстановить из копии',
  'backup.saved': 'Резервная копия сохранена.',
  'backup.preview': 'Копия от {date}: файлов — {files}, локальных настроек — {settings}. Текущие данные будут заменены, а перед этим SkillCue создаст копию для отката.',
  'backup.confirm': 'Подтвердить восстановление',
  'backup.cancel': 'Не восстанавливать',
  'backup.restarting': 'Данные восстановлены. SkillCue перезапускается…',
  'backup.error': 'Не удалось обработать резервную копию.',
  ...
  'settings.kb.candidateFollowUp': 'Продолжить с моей последней репликой',
  ...
  'billing.plan.basic.summary': 'Подготовка, практика и отклики без оверлея.',
  'billing.plan.max.summary': 'Всё приложение, включая оверлей и скрытый режим.',
  ...
  'billing.feat.fullCycle': 'Полный цикл подготовки к вакансии',
  'billing.feat.vacancyPractice': 'Разбор вакансии, практика и мок-интервью',
  ...
  'billing.feat.overlay': 'Оверлей поверх Zoom, Meet и браузера',
  ...
  'plan.renew': 'Продлить доступ',
  'plan.upgrade': 'Перейти на Максимум',
  'plan.downgradeUnavailable': 'Доступен после окончания Максимум',
  'plan.checkoutNote':
  ...
  'license.keyPrompt':
    'Введите ключ SKILLCUE-…, чтобы активировать или сменить тариф.',
