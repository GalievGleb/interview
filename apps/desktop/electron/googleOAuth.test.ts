...
describe('Google desktop OAuth', () => {
  it('sends the desktop client credential with PKCE only to the Google token endpoint', async () => {
    const result = await runGoogleDesktopOAuth({
      clientId, clientSecret: 'test-desktop-client-secret', state: 'expected-state', verifier,
      openExternal: async (url) => { expect(url).not.toContain('test-desktop-client-secret'); },
      createListener: async () => listener('http://127.0.0.1:43123/oauth2/callback?code=auth-code&state=expected-state'),
      fetchImpl: async (url, init) => {
        expect(url).toBe('https://oauth2.googleapis.com/token');
        const body = new URLSearchParams(String(init?.body));
        expect(body.get('client_secret')).toBe('test-desktop-client-secret');
        expect(body.get('code_verifier')).toBe(verifier);
        return new Response(JSON.stringify({ id_token: 'test-id-token' }));
      },
    });
    expect(result).toEqual({ idToken: 'test-id-token' });
  });
  it('completes OAuth through the real loopback listener with its ephemeral port', async () => {
    const result = await runGoogleDesktopOAuth({
      clientId,
      ...
      fetchImpl: async () => new Response(JSON.stringify({ id_token: 'test-id-token' })),
    });
    expect(result).toEqual({ idToken: 'test-id-token' });
  });
