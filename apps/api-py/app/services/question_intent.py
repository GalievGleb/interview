...
_EXPERIENCE_RE = re.compile(
    r"(?:расскаж\w*[^.?!\n]{0,50}\b(?:про|о)\s+(?:(?:сво|ваш|тво)\w+\s+)?опыт|"
    r"(?:ваш|твой|свой)\s+опыт|"
...
_PRACTICAL_RE = re.compile(
    ...
    r"проверял\w*|тестировал\w*|запускал\w*|работал\w*|делал\w*|писал\w*)|"
    r"как\s+(?:у\s+(?:вас|тебя)|в\s+(?:ваш\w*|тво\w*)\s+(?:работ\w*|проект\w*|команд\w*|компани\w*))"
    r"\s+(?:был\w*|были)\s+(?:устроен\w*|настроен\w*|организован\w*|реализован\w*)|"
    r"(?:в|на)\s+(?:(?:сво\w*|ваш\w*|тво\w*)\s+)?работ\w*|на\s+проект\w*|"
