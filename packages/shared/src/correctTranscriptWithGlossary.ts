...
const QA_CONTEXT_RE =
  /(?:что\s+такое|расскаж|объясн|опиш|назов|перечисл|чем\s+.+\s+отлича|принцип|тестир|интервью|\bqa\b|\baqa\b|какие|какой|как\s+ты|проверял|настраивал|использовал)/iu;
