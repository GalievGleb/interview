...
const PRACTICAL_RE =
  /(?:как\s+ты\s+(?:применял\w*|использовал\w*|настраивал\w*|проверял\w*|запускал\w*|работал\w*|делал\w*|писал\w*)|ты\s+сам\w*\s+(?:настраивал\w*|делал\w*|писал\w*|использовал\w*|настраивал\w*)|сам\s+настраивал\w*|как\s+вы\s+(?:применял\w*|использовал\w*|настраивал\w*|проверял\w*|тестировал\w*|запускал\w*|работал\w*|делал\w*|писал\w*)|как\s+(?:у\s+(?:вас|тебя)|в\s+(?:ваш\w*|тво\w*)\s+(?:работ\w*|проект\w*|команд\w*|компани\w*))\s+(?:был\w*|были)\s+(?:устроен\w*|настроен\w*|организован\w*|реализован\w*)|(?:в|на)\s+(?:(?:сво[а-яё]*|ваш[а-яё]*|тво[а-яё]*)\s+)?работ[а-яё]*|на\s+проект\w*|how\s+(?:did|do|have)\s+you\s+(?:use|apply|set\s*up|configure|implement|test|work)|have\s+you\s+(?:ever\s+)?(?:used|worked\s+with|built|set\s*up)|in\s+your\s+work|on\s+your\s+project)/iu;
