...
Для macOS ссылка `releases/latest` не подходит, пока Mac-сборка опубликована как
предварительный релиз: GitHub оставляет `latest` на последнем обычном релизе.
Поэтому Mac-ссылки в `landing/` указывают на конкретные проверенные версии и
подписаны номером версии и архитектурой. После нового Mac-релиза обновите эти
ссылки и задеплойте `landing/`. Браузер Apple Silicon может сообщать `MacIntel`,
поэтому кнопка сайта предлагает выбрать версию вручную. Серверные Mac-редиректы
`/downloads/SkillCue-macOS-*.dmg` также ведут через `latest`; не используйте их
для предварительного Mac-релиза.

Публичная бета-линия с 26 августа 2026 года использует `0.1.x`: первый релиз
линии — `0.1.0`, следующие исправления выпускаются как `0.1.1`, `0.1.2`, …
